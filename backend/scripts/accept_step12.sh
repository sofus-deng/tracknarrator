...
# seed
# seed
SEED=$(curl -sS -X POST http://127.0.0.1:8000/dev/seed -H 'Content-Type: application/json' --data-binary @../fixtures/bundle_sample_barber.json)
SID=$(echo "$SEED" | python3 -c "import json,sys; print(json.loads(sys.stdin.read()).get('session_id','barber'))")
# viz endpoint contract
curl -sS http://127.0.0.1:8000/session/"$SID"/viz | grep -E '"lap_delta_series"|"section_box"' >/dev/null
